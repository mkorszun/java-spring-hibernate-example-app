...
In this tutorial we're going to show you how to create example Spring/MVC/Hibernate application, integrate it with MySQLs Add-on, deploy it on embedded Jetty server and run on [cloudControl](https://www.cloudcontrol.com/). Check out the [buildpack-java](https://github.com/cloudControl/buildpack-java) for supported features.
...
Download [Spring Roo](http://www.springsource.org/download/community), extract it and use `bin/roo.sh` script to create `Petclinic` example application:
...
Go to application context configuration file: `src/main/resources/META-INF/spring/applicationContext.xml` and modify datasource `username`, `password` and `url` properties:
